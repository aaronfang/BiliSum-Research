...
type CookieExportResult = {
  cookiesFile: string;
  cookieCount: number;
  browser: string;
};

type CookieCaptureTarget = {
  loginUrl: string;
  windowTitle: string;
  partition: string;
  browserName: string;
...
function getCookieExportPath(target: CookieCaptureTarget) {
  return path.join(currentLocalDataRoot(), "data", "cookies", target.cookieFile);
}
...
async function collectTargetCookies(loginWindow: BrowserWindow, target: CookieCaptureTarget) {
  const cookieStore = loginWindow.webContents.session.cookies;
  const cookieMap = new Map<string, Electron.Cookie>();
  for (const url of target.collectUrls) {
    const cookies = await cookieStore.get({ url });
    for (const cookie of cookies) {
      const domain = String(cookie.domain || "");
      if (!target.domainHints.some((hint) => domain.includes(hint))) {
        continue;
      }
...
async function exportTargetCookies(cookies: Electron.Cookie[], target: CookieCaptureTarget): Promise<CookieExportResult> {
  const exportableCookies = cookies.filter((cookie) => {
    if (!cookie.name || !cookie.domain) {
      return false;
    }
    const domain = String(cookie.domain);
    return target.domainHints.some((hint) => domain.includes(hint));
  });
  const cookiesFile = getCookieExportPath(target);
  fs.mkdirSync(path.dirname(cookiesFile), { recursive: true });
  const lines = [
    "# Netscape HTTP Cookie File",
    `# Generated by BiliSum from the in-app ${target.browserName} login window.`,
    ...exportableCookies
  ...
  return { cookiesFile, cookieCount: exportableCookies.length, browser: target.browserName };
}

const BILIBILI_COOKIE_TARGET: CookieCaptureTarget = {
  loginUrl: "https://passport.bilibili.com/login",
  windowTitle: "登录 Bilibili - BiliSum",
  partition: "persist:briefvid-bilibili-login",
  browserName: "bilibili",
  cookieFile: "bilibili.txt",
...
const YOUTUBE_COOKIE_TARGET: CookieCaptureTarget = {
  loginUrl: "https://accounts.google.com/ServiceLogin?service=youtube&continue=https%3A%2F%2Fwww.youtube.com%2Fsignin%3Faction_handle_signin%3Dtrue%26app%3Ddesktop%26hl%3Dzh-CN%26next%3Dhttps%253A%252F%252Fwww.youtube.com%252F",
  windowTitle: "登录 YouTube - BiliSum",
  partition: "persist:briefvid-youtube-login",
  browserName: "youtube",
  cookieFile: "youtube.txt",
  domainHints: ["youtube.com", "google.com"],
  loginCookieNames: ["LOGIN_INFO", "SID", "SAPISID", "SSID", "HSID", "APISID"],
  collectUrls: [
...
async function openLoginAndCaptureCookies(target: CookieCaptureTarget): Promise<CookieExportResult> {
  return new Promise((resolve, reject) => {
    ...
    const tryCapture = async () => {
      try {
        const cookies = await collectTargetCookies(loginWindow, target);
        const hasLoginCookie = cookies.some(
          (cookie) => target.loginCookieNames.includes(String(cookie.name)) && Boolean(cookie.value),
        );
        if (!hasLoginCookie) {
          return;
        }
        const result = await exportTargetCookies(cookies, target);
        settle(() => {
    ...
    loginWindow.on("closed", () => {
      settle(() => reject(new Error(`${target.browserName === "youtube" ? "YouTube" : "B 站"}登录窗口已关闭，尚未捕获到登录态。`)));
    });
    ...
    loginWindow.loadURL(target.loginUrl).catch((error) => {
      settle(() => reject(error));
    });
  });
}

function openBilibiliLoginAndCaptureCookies() {
  return openLoginAndCaptureCookies(BILIBILI_COOKIE_TARGET);
}

function openYoutubeLoginAndCaptureCookies() {
  return openLoginAndCaptureCookies(YOUTUBE_COOKIE_TARGET);
}
...
function terminateBackendProcessTree(pid: number): Promise<void> {
  if (!pid || pid <= 0) {
  ...
  if (process.platform !== "win32") {
    try {
      ...
      // The process may have exited before shutdown reached this path.
    }
    return Promise.resolve();
  }
...
async function stopBackend(): Promise<BackendStatus> {
  ...
  void terminateBackendProcessTree(current.pid ?? 0);
  updateBackendStatus({
    running: false,
    ready: false,
    pid: null,
...
function registerIpcHandlers() {
  ...
  ipcMain.handle("desktop:youtube:capture-login-cookies", async () => openYoutubeLoginAndCaptureCookies());
